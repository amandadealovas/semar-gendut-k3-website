    ...
    <div class="about-intro-grid">
      ...
      <div>
        <div style="background:var(--navy);border-radius:16px;padding:36px;color:var(--white);">
          <h3 style="font-family:'Barlow Condensed',sans-serif;font-size:1.6rem;color:var(--cyan);text-transform:uppercase;margin-bottom:20px;">Identitas Perusahaan</h3>
          <div style="display:flex;flex-direction:column;gap:16px;">
            <?php
            $company_info = [
              ['📋','Nama Resmi','PT. Semar Gendut Indonesia'],
              ['🏢','Bidang Usaha','Transportasi Darat — Bus Premium'],
              ['📍','Kantor Pusat','Jl. Veteran No. 88, Malang, Jawa Timur'],
              ['📅','Tahun Berdiri','2024'],
              ['🌐','Wilayah Operasi','Regional & Nasional (Pulau Jawa)'],
              ['🔖','Status','Perusahaan Swasta Nasional (PT)'],
            ];
            ...
            <div style="display:flex;gap:12px;align-items:flex-start;padding-bottom:14px;border-bottom:1px solid rgba(255,255,255,0.07);">
              <span style="font-size:1.2rem;flex-shrink:0;margin-top:2px;"><?= $info[0] ?></span>
              <div>
                <div style="font-size:0.72rem;color:rgba(255,255,255,0.4);text-transform:uppercase;letter-spacing:0.06em;margin-bottom:2px;"><?= $info[1] ?></div>
                <div style="font-size:0.9rem;color:var(--white);font-weight:500;"><?= $info[2] ?></div>
              </div>
            </div>
            <?php endforeach; ?>
          </div>
...
<section class="section section-gray" id="visi">
  <div class="container">
    <h2 class="section-title center" style="display:block;text-align:center;">Visi &amp; <span>Misi</span></h2>
    <div class="vision-mission-grid">
      <div class="vm-card">
        <h3>🎯 Visi</h3>
        <p>Menjadi pemimpin ekosistem perjalanan darat premium di Indonesia dengan menghadirkan kemewahan, kenyamanan, <em>hospitality</em> unggul, dan standar keselamatan terbaik yang diakui secara nasional.</p>
      </div>
      <div class="vm-card">
        <h3>🚀 Misi</h3>
        <ul>
...
<section class="section">
  <div class="container">
    <h2 class="section-title center" style="display:block;text-align:center;">Nilai-Nilai <span>Perusahaan</span></h2>
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:20px;margin-top:40px;">
      <?php
      $values = [
        ['🛡️','Keselamatan','Safety First — tidak ada target bisnis yang lebih penting dari keselamatan manusia'],
        ['⭐','Integritas','Transparan, jujur, dan bertanggung jawab dalam setiap keputusan dan tindakan'],
        ['🤝','Pelayanan Prima','Kepuasan pelanggan adalah tolak ukur keberhasilan kami, bukan sekadar keuntungan'],
        ['🌱','Berkelanjutan','Operasional yang ramah lingkungan dan bertanggung jawab sosial kepada masyarakat'],
        ['💡','Inovasi','Terus belajar, beradaptasi, dan mengadopsi teknologi terbaik untuk kemajuan layanan'],
        ['👥','Kolaborasi','Membangun tim yang solid, saling mendukung, dan tumbuh bersama perusahaan'],
      ];
      foreach ($values as $v): ?>
      <div style="text-align:center;padding:24px 16px;background:var(--gray-light);border-radius:10px;transition:all 0.3s;" onmouseover="this.style.background='var(--navy)';this.style.color='white'" onmouseout="this.style.background='var(--gray-light)';this.style.color=''">
        <div style="font-size:2.2rem;margin-bottom:10px;"><?=$v[0]?></div>
        <h4 style="font-family:'Barlow Condensed',sans-serif;font-size:1rem;text-transform:uppercase;margin-bottom:8px;color:inherit;"><?=$v[1]?></h4>
        <p style="font-size:0.8rem;line-height:1.6;color:inherit;opacity:0.7;"><?=$v[2]?></p>
      </div>
      <?php endforeach; ?>
    </div>
  </div>
